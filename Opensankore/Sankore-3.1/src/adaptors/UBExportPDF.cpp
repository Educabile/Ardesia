...
void UBExportPDF::persist(UBDocumentProxy* pDocumentProxy)
{
    ...
    QString filename = askForFileName(pDocumentProxy, tr("Salva come PDF File"));
...
void UBExportPDF::persistsDocument(UBDocumentProxy* pDocumentProxy, const QString& filename)
{
    ...
    qDebug() << "Esportazione documento to PDF" << filename;
    ...
    for(int pageIndex = 0 ; pageIndex < existingPageCount; pageIndex++)
    {
        ...
        UBApplication::showMessage(tr("Salvataggio pagina %1 di %2").arg(pageIndex + 1).arg(existingPageCount));
