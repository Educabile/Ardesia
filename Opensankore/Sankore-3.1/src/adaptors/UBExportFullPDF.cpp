...
void UBExportFullPDF::saveOverlayPdf(UBDocumentProxy* pDocumentProxy, const QString& filename)
{
    ...
    //PDF
    qDebug() << "Esportazione documento to PDF Merger" << filename;
    QPrinter pdfPrinter;
...
void UBExportFullPDF::persist(UBDocumentProxy* pDocumentProxy)
{
    ...
    QString filename = askForFileName(pDocumentProxy, tr("Esposta come PDF File"));

    if (filename.length() > 0)
    {
        QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
        if (mIsVerbose)
            UBApplication::showMessage(tr("Esportazione documento"));
