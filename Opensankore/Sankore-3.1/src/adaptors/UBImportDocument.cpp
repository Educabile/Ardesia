...
UBDocumentProxy* UBImportDocument::importFile(const QFile& pFile, const QString& pGroup)
{
    ...
    QFileInfo fi(pFile);
    UBApplication::showMessage(tr("Carico il file %1...").arg(fi.baseName()), true);
	...
	if(!extractFileToDir(pFile, path, documentRootFolder)){
        UBApplication::showMessage(tr("Caricamento %1 fallito.").arg(fi.baseName()));
		return NULL;
	}
    ...
    UBDocumentProxy* newDocument = UBPersistenceManager::persistenceManager()->createDocumentFromDir(documentRootFolder
                                                                                                     ...
                                                                                                     , addTitlePage
                                                                                                     , true);
    UBApplication::showMessage(tr("Caricamento completato con successo."));
	return newDocument;
}
...
bool UBImportDocument::addFileToDocument(UBDocumentProxy* pDocument, const QFile& pFile)
{
    ...
    if (!extractFileToDir(pFile, path, documentRootFolder))
    {
        UBApplication::showMessage(tr("Caricamento di %1 fallito.").arg(fi.baseName()));
        return false;
    }
