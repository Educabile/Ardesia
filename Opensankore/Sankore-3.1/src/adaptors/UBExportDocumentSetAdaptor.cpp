...
void UBExportDocumentSetAdaptor::persist(UBDocumentProxy* pDocumentProxy)
{
    ...
    if (pDocumentProxy) {
        ...
        if (!treeViewParentIndex.isValid()) {
            ...
            UBApplication::showMessage(tr("salvataggio fallita..."));
            return;
        }
        filename = askForFileName(pDocumentProxy, tr("Salva come UBX File"));
        ...
        filename = askForFileName(&proxy, tr("Salva come UBX File"));
    }
    ...
    if (filename.length() > 0)
    {
        ...
        if (mIsVerbose)
            UBApplication::showMessage(tr("Salvataggio documento.."));

         if (persistData(treeViewParentIndex, filename)) {
             if (mIsVerbose) {
                 UBApplication::showMessage(tr("Salvataggio completato."));
             }
         } else {
             if (mIsVerbose) {
                 UBApplication::showMessage(tr("Salvataggio fallito."));
             }
         }
...
bool UBExportDocumentSetAdaptor::persistData(const QModelIndex &pRootIndex, QString filename)
{
    ...
    {
        qWarning("Salvataggio fallito. Causa: zip.open(): %d", zip.getZipError());
        return false;
    }
...
{
    return tr("Salva in formato Ardesia UBX");
}
...
//        cleaner.cure(pDocumentProxy->persistencePath());
        ...
        {
            qWarning("Salvataggio fallito. Causa: zip.close(): %d", zip.getZipError());
        }
    }
