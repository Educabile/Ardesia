...
{
        UBExportDocument::tr("Pagina"); // dummy slot for translation
}
...
void UBExportDocument::persist(UBDocumentProxy* pDocumentProxy)
{
    ...
    QString filename = askForFileName(pDocumentProxy, tr("Salva come UBZ File"));
    ...
    if (filename.length() > 0)
    {
        ...
        if (mIsVerbose)
            UBApplication::showMessage(tr("Salvataggio in corso..."));

        persistsDocument(pDocumentProxy, filename);

        if (mIsVerbose)
            UBApplication::showMessage(tr("Export completato."));
...
void UBExportDocument::processing(const QString& pObjectName, int pCurrent, int pTotal)
{
    ...
    if (mIsVerbose)
        UBApplication::showMessage(tr("Salvataggio %1 %2 di %3").arg(localized).arg(pCurrent).arg(pTotal));
}
...
{
    return tr("Salva in Formato Ardesia");
}
