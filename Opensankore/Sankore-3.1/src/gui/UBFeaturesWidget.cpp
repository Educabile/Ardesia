    ...
    : QListView(parent)
{
    ...
    textRestoreOneFile = tr("Ripristina");
    textRestoreManyFile = tr("Ripristina %1 elementi");
...
UBFeaturesNewFolderDialog::UBFeaturesNewFolderDialog(QWidget *parent) : QWidget(parent)
  ...
  , labelText(tr("Inserisci il nome della cartella"))
{
...
{
    mProgressBar->setFormat(tr("Caricamento ") + str + " (%p%)");
}
    ...
    , mpElement(NULL)
{
    ...
    mpSetAsDefaultBackgroundButton->setText(tr("Imposta come sfondo di default"));
    mpButtonLayout->addWidget(mpSetAsDefaultBackgroundButton);

    mpAddToLibButton = new UBFeatureItemButton();
    mpAddToLibButton->setText(tr("Aggiungi alla libreria"));
    mpButtonLayout->addWidget(mpAddToLibButton);

    mpButtonLayout->addStretch(1);

    mpObjInfoLabel = new QLabel(tr("Informazioni"));
