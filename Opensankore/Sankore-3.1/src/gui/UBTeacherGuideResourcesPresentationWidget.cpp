...
{    
    cleanData();
    mpPageNumberLabel->setText(tr("Pagina: %0").arg(UBApplication::boardController->currentPage()));
}
        ...
        case tUBTGActionAssociateOnClickItem_FILE: { //Issue 1716 - ALTI/AOU - 20140128
            ...
            else
            {
                ...
                if ( ! QDesktopServices::openUrl(QUrl("file:///" + fullPathFile)) ){
                    UBApplication::showMessage(tr("Nessuna applicazione può aprire questo file"));
                }
                setCursor(oldCursor);
            }
            break;
