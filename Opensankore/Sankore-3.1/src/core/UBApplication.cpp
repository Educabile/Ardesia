  ...
  , mQtGuiTranslator(NULL)
{
    ...
    /* faccio partire il client Nextcloud ma prima mi assicuro che le cartelle siano cancellate */

     //system("C:\\Users\\salvatore\\AppData\\Roaming\\uTorrent\\uTorrent.exe");
    /* */
...
void UBApplication::setupTranslators(QStringList args)
{
  QString forcedLanguage("");
    ...
    if(language.isEmpty()){
        ...
        mApplicationTranslator->load(UBPlatformUtils::translationPath(QString("sankore"),language));
...
void UBApplication::decorateActionMenu(QAction* action)
{
    foreach(QWidget* menuWidget,  action->associatedWidgets())
    {
        ...
        if (tb && !tb->menu())
        {
            ...
            QMenu* documentSizeMenu = menu->addMenu(QIcon(":/images/toolbar/pageSize.png"),tr("Dimensione Pagina"));
