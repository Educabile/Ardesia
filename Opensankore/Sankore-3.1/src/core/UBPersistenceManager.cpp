...
QString UBPersistenceManager::generateUniqueDocumentPath(const QString& baseFolder)
{
    ...
    return baseFolder + QString("/Documento Ardesia %1").arg(dirName);
}
...
void UBPersistenceManager::checkIfDocumentRepositoryExists()
{
    ...
    if (!rp.exists())
    {
        ...
        UBApplication::mainWindow->warning(tr("Directory non disponibile"),tr("Acesso alla directory non disponibile '%1'. L'applicazione sarà riavviata.").arg(humanPath));
