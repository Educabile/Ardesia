...
{
    productWebUrl =  new UBSetting(this, "App", "ProductWebAddress", "http://143.225.48.253/nextcloud/");
...
//    documentSizes.insert(DocumentSizeRatio::Ratio16_9, QSize((mainScreenGeometry.height() * 16.f / 9.f), mainScreenGeometry.height())); // 1.77
    ...
    bool defaultShowPageImmediatelyOnMirroredScreen = false;
...
QString UBSettings::userDataDirectory()
{
   ...
   /* static QString dataDirPath = "";
    ...
    /* Nella stringa ownCloud_Path sarà presente la path di installazione
         * di ownCloud in base ai differenti sistemi operativi.
         */

        QString nextCloud_Path;

        /* Determiniamo il sistema operativo e di conseguenza "costruiamo" la path di installazione */
        #ifdef Q_OS_WIN
            QString User_Home = getenv("HOMEPATH");
            nextCloud_Path = User_Home + "/Nextcloud/Ardesia";
        #elif defined(Q_OS_LINUX)
            QString home_dir = getenv("HOME");
            nextCloud_Path = home_dir + "/Nextcloud/Ardesia";
        #elif defined(Q_OS_MACOS)
            Qstring home_dir; /* Non sono sicuro su quale sia la path di installazione su Mac OS --- Forse in ~/Applications/ownCloud non sono sicuro*/

            next
                    Cloud_Path = home_dir + "/Nextcloud/Ardesia";
        #endif

        /* Restituisco la path di installazione */
...
QString UBSettings::userImageDirectory()
{
    ...
    if(imageDirectory.isEmpty()){
        ...
        QString User_Home = getenv("HOMEPATH");
        imageDirectory = User_Home + "/Nextcloud/Ardesia/Immagini/";
...
QString UBSettings::userVideoDirectory()
{
    ...
    if(videoDirectory.isEmpty()){
       ...
       /* videoDirectory = QDesktopServices::storageLocation(QDesktopServices::MoviesLocation);
        ...
        QString User_Home = getenv("HOMEPATH");
        videoDirectory = User_Home + "/Nextcloud/Ardesia/Video/";
...
QString UBSettings::userAudioDirectory()
{
    ...
    if(audioDirectory.isEmpty()){
        ...
        QString User_Home = getenv("HOMEPATH");
        audioDirectory = User_Home + "/Nextcloud/Ardesia/Audio/";
...
QString UBSettings::userPodcastRecordingDirectory()
{
    ...
    if(dirPath.isEmpty()){
        ...
        QString User_Home = getenv("HOMEPATH");
        dirPath = User_Home + "/Nextcloud/Ardesia/Podcasts/";
...
QString UBSettings::userDocumentDirectory()
{
    ...
    static QString lessonsDirectory = "";
    static QString materialsDirectory = "";
    if(documentDirectory.isEmpty()){
        documentDirectory = userDataDirectory() + "/backup_documenti";
        lessonsDirectory = userDataDirectory() + "/lezioni";
        materialsDirectory = userDataDirectory() + "/documenti";
        checkDirectory(lessonsDirectory);
        checkDirectory(documentDirectory);
        checkDirectory(materialsDirectory);
    }
...
QString UBSettings::userBookmarkDirectory()
{
    ...
    if(bookmarkDirectory.isEmpty()){
        bookmarkDirectory = userDataDirectory() + "/servizi-web";
...
QString UBSettings::userAnimationDirectory()
{
    ...
    if(animationDirectory.isEmpty()){
        animationDirectory = userDataDirectory() + "/documenti";//"/animationUserDirectory";
