...
//                                         ,QImage(":images/libpalette/folder")    /*FlashCategory.svg")*/                //Icon
//                                         , tr("Documenti")//("Animazioni")                                             //Translation name
//                                         , QUrl::fromLocalFile(UBSettings::settings()->applicationAnimationsLibraryDirectory()
...
//                             .insertr(CategoryData::UserDefined, UBFeature::NO_P));
    ...
    /* appData = CategoryData(CategoryData::PathData()   //Static library paths for Applications category. Scanning data
                           ...
                           , UBFeature(rootData.categoryFeature().getFullVirtualPath() + "/Applications"   //Virtual Path
                                       ...
                                       , tr("Applicazioni")                                             //Translation name
                                       , QUrl::fromLocalFile(UBSettings::settings()->
  ...
  /*  interactivityData = CategoryData(CategoryData::PathData() //Static library paths for Interactivities category. Scanning data
                                     ...
                                     , UBFeature(rootData.categoryFeature().getFullVirtualPath() + "/Interactivities"     //Virtual Path
                                                 ...
                                                 , tr("Interattive")                                               //Translation name
                                                 , QUrl::fromLocalFile(UBSettings::settings()->
    ...
    /*webSearchData = CategoryData(CategoryData::PathData()  //Static library paths for Web search category. Scanning data
                                 ...
                                 , UBFeature(rootData.categoryFeature().getFullVirtualPath() + "/Web search"   //Virtual Path
                                             ...
                                             , tr("Ricerca Web")                                             //Translation name
                                             , QUrl::fromLocalFile(UBSettings::settings()->
    ...
    bookmarkData = CategoryData(CategoryData::PathData() //Path for incoming user web content.
                                ...
                                , UBFeature(rootData.categoryFeature().getFullVirtualPath()
                                            + "/servizi-web"                //Virtual Path
                                            , QImage(":images/libpalette/BookmarkCategory.svg")         //Icon
                                            , tr("Servizi Web"/*"Segnalibri"*/)                                             //Translation name
    ...
    if(QDir(path).exists()){
        ...
        if(name != entry.filename){
            bool move = UBApplication::mainWindow->yesNoQuestion(
                            tr("Cambio Cartella"),
                            tr("Ci sono cartelle col nome uguale.")
                            + "\n\n"
                            + tr("La cartella sarà rinominata in %1").arg(name)
                            + "\n\n"
                            + tr("vuoi procedere ?")
                        );
            if(move){
            ...
            UBApplication::mainWindow->warning(
                tr("Cambio cartella"),
                tr("Ci sono cartelle con lo stesso nome: %1. Per favore eliminare i conflitti.").arg(entry.originalVirtualPath));
...
bool UBFeaturesController::restoreFileFeature(UBFeature& feature, RegisteryEntry entry)
{
    ...
    if(QFile(path).exists()){
        ...
        if(name != entry.filename){
            bool move = UBApplication::mainWindow->yesNoQuestion(
                            tr("Cambio file"),
                            tr("Ci sono file con lo stesso nome.")
                            + "\n\n"
                            + tr("il fiel sarà spostato e rinominato %1").arg(name)
                            + "\n\n"
                            + tr("vuoi procedere ?")
                        );
            ...
            UBApplication::mainWindow->warning(
                tr("Spostamento files"),
                tr("Ci sono files con lo stesso nome %1, nella cartella. Per favore cancella dei files.").arg(entry.originalVirtualPath));
            hasBeenRestored = false;
        }
    }else{
        UBFeature folder = this->getFeatureByPath(entry.originalVirtualPath);
