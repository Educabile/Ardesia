...
UBDocumentReplaceDialog::UBDocumentReplaceDialog(const QString &pIncommingName, const QStringList &pFileList, QWidget *parent, Qt::WindowFlags pFlags)
    ...
    , acceptText(tr("Accetta"))
    , replaceText(tr("Sostituisci"))
    , cancelText(tr("Cancella"))
    , mLabelText(0)
{
...
{
    return tr("Il nome %1 è già in uso.\nIl documento già presente verrà sovrascritto.\Per creare un nuovo documento usare un nome diverso.")
            .arg(documentName);
}
  ...
  , mRootNode(0)
{
    ...
    UBDocumentTreeNode *myDocsNode = new UBDocumentTreeNode(UBDocumentTreeNode::Catalog, UBPersistenceManager::myDocumentsName, tr("Documenti"));
    rootNode->addChild(myDocsNode);
    UBDocumentTreeNode *modelsNode = new UBDocumentTreeNode(UBDocumentTreeNode::Catalog, UBPersistenceManager::modelsName, tr("Modelli"));
    rootNode->addChild(modelsNode);
    UBDocumentTreeNode *trashNode = new UBDocumentTreeNode(UBDocumentTreeNode::Catalog, trashName, tr("Cestino"));
    rootNode->addChild(trashNode);
    UBDocumentTreeNode *untitledDocumentsNode = new UBDocumentTreeNode(UBDocumentTreeNode::Catalog, UBPersistenceManager::untitledDocumentsName, tr("Documenti Senza Titolo"));
...
UBDocumentController::UBDocumentController(UBMainWindow* mainWindow)
   ...
   , mTrashTi(0)
   , mDocumentTrashGroupName(tr("Cestino"))
   , mDefaultDocumentGroupName(tr("Documenti senza nome"))
...
void UBDocumentController::setupViews()
{
    ...
    if (!mDocumentWidget)
    {
        ...
        mAddFolderOfImagesAction = addMenu->addAction(tr("Aggiungi cartella di immagini"));
        mAddImagesAction = addMenu->addAction(tr("Aggiungi immagini"));
        mAddFileToDocumentAction = addMenu->addAction(tr("Aggiungi pagine da un file"));
...
void UBDocumentController::importFile()
{
    ...
    QString filePath = QFileDialog::getOpenFileName(mParentWidget, tr("Apro il file"),
    ...
    if (fileInfo.suffix().toLower() == "ubx") {
        ...
        if (filePath.length() > 0)
        {
            ...
            showMessage(tr("Importo il file %1...").arg(fileInfo.baseName()), true);
...
void UBDocumentController::addFolderOfImages()
{
    ...
    if (document)
    {
        ...
        QString imagesDir = QFileDialog::getExistingDirectory(mParentWidget, tr("Importa tutte le immagini dalla cartella"), defaultPath);
        ...
        if (imagesDir.length() > 0)
        {
            ...
            {
                showMessage(tr("La cartella non contiene immagini"));
...
{
    QString defaultPath = UBSettings::settings()->lastImportFilePath->get().toString();
    QString filePath = QFileDialog::getOpenFileName(mParentWidget, tr("Apro il file: "), defaultPath, UBDocumentManager::documentManager()->importFileFilter(true));
    ...
    if (filePath.length() > 0)
    {
        ...
        showMessage(tr("Importazione del file %1...").arg(fileInfo.baseName()), true);
        ...
        {
            showMessage(tr("Importazione fallita ... "));
        }
    }
...
void UBDocumentController::thumbnailPageDoubleClicked(QGraphicsItem* item, int index)
{
    ...
    if (selectedIndex.isValid()) {
        if (docModel->inModel(selectedIndex)) {
            UBApplication::showMessage(tr("Documento non modificabile. Copiarlo in \"My documents\" per editarlo"));
            return;
...
bool UBDocumentController::isOKToOpenDocument(UBDocumentProxy* proxy)
{
    ...
    {
        if (UBApplication::mainWindow->yesNoQuestion(tr("Apro il documento"),
                tr("il documento '%1' è stato generato con una nuova versione del software(%2). Potrebbe esserci perdita di informazione, vuoi procedere?")
...
void UBDocumentController::addImages()
{
    ...
    if (document)
    {
        ...
        QStringList images = QFileDialog::getOpenFileNames(mParentWidget, tr("Aggiungi tutte le immagini al documento"),
                defaultPath, tr("Tutte le immagini (%1)").arg(extensions));
        ...
        if (images.length() > 0)
        {
            ...
            if (importedImageNumber == 0)
            {
                UBApplication::showMessage(tr("Non ci sono immagini da importare"));
                UBApplication::applicationController->showDocument();
            }
            else
            {
...
void UBDocumentController::updateActions()
{
    ...
    case DeletePage :
        ...
        mMainWindow->actionDelete->setText(tr("Cestino"));
        break;
    case CompleteDelete :
        mMainWindow->actionDelete->setIcon(QIcon(":/images/toolbar/deleteDocument.png"));
        mMainWindow->actionDelete->setText(tr("Elimina"));
        break;
    case EmptyFolder :
        mMainWindow->actionDelete->setIcon(QIcon(":/images/trash.png"));
        mMainWindow->actionDelete->setText(tr("Vuota"));
        break;
    case EmptyTrash :
        mMainWindow->actionDelete->setIcon(QIcon(":/images/toolbar/deleteDocument.png"));
        mMainWindow->actionDelete->setText(tr("Vuota"));
...
void UBDocumentController::deletePages(QList<QGraphicsItem *> itemsToDelete)
{
    if (itemsToDelete.count() > 0)
    {
        ...
        if(UBApplication::mainWindow->yesNoQuestion(tr("Elimina pagina"),tr("Questa azione è irreversibile!") +"\n\n" + tr("ArSei sicuro di voler rimuovere la pagina %n dal documento '%1'?", "", sceneIndexes.count()).arg(proxy->metaData(UBSettings::documentName).toString())))
        {
...
void UBDocumentController::refreshDocumentThumbnailsView(UBDocumentContainer*)
{
    ...
    if (currentDocumentProxy)
    {
        for (int i = 0; i < currentDocumentProxy->pageCount(); i++)
        {
            ...
            if(pageIndex)
                labels << tr("Pagina %1").arg(pageIndex);
            else
                labels << tr("Pagina Iniziale");
