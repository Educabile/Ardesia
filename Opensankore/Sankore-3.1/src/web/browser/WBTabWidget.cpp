...
{
    QMenu menu;
    menu.addAction(tr("Nuovo &Tab"), this, SIGNAL(newTab()), QKeySequence::AddTab);
    int index = tabAt(position);
    if (-1 != index) {
        QAction *action = menu.addAction(tr("Copia Tab"), this, SLOT(cloneTab()));
        action->setData(index);

        menu.addSeparator();

        action = menu.addAction(tr("&Chiudi Tab"), this, SLOT(closeTab()), QKeySequence::Close);
        action->setData(index);

        action = menu.addAction(tr("Chiudi &Other Tab"), this, SLOT(closeOtherTabs()));
        action->setData(index);

        menu.addSeparator();

        action = menu.addAction(tr("Ricarica Tab"), this, SLOT(reloadTab()), QKeySequence::Refresh);
        action->setData(index);
    }
    else {
        menu.addSeparator();
    }
    menu.addAction(tr("Ricarica tutti i Tab"), this, SIGNAL(reloadAllTabs()));
...
WBWebView *WBTabWidget::newTab(bool makeCurrent)
{
    ...
    if (count() == 0)
    {
        ...
        addTab(emptyWidget, tr("(Senza Titolo)"));
        connect(this, SIGNAL(currentChanged(int)), this, SLOT(currentChanged(int)));
        return 0;
    }
