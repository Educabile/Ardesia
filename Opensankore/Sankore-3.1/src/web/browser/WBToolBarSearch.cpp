...
void WBToolbarSearch::aboutToShowMenu()
{
    ...
    if (list.isEmpty())
    {
        m->addAction(tr("Nessuna ricerca recente"));
        return;
    }

    QAction *recent = m->addAction(tr("Ricerche recenti"));
    ...
    m->addSeparator();
    m->addAction(tr("Pulisci ricerche recenti"), this, SLOT(clear()));
}
