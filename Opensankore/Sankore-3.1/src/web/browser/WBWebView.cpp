...
void WBWebPage::handleUnsupportedContent(QNetworkReply *reply)
{
    ...
    if (isPDF)
    {
        ...
        messageBox.setText(tr("Scarica documento PDF: preferisci scaricare il file PDF o aggiungerlo all'attuale documento?"));
    ...
    {
        QString title = tr("Errore nel caricamento della pagina: %1").arg(reply->url().toString());
        html = QString(QLatin1String(file.readAll()))
                        .arg(title)
...
void WBWebView::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    if (!r.linkUrl().isEmpty())
    {
        ...
        menu.addAction(tr("Apri in un nuovo Tab"), this, SLOT(openLinkInNewTab()));
        menu.addSeparator();
